...
const Tablist = ({ experiences }: Props) => {
  ...
  const { role, company, companyUrl, started,  tasks } =
    ...
    <div className="flex flex-col sm:flex-row text-sm md:text-base gap-6 md:gap-10 min-h-[250px]">
      ...
      <div className="font-mono text-xs sm:text-sm relative flex justify-start sm:flex-col overflow-scroll sm:overflow-auto sm:min-w-[150px]">
        {experiences.map(({ company }, i) => (
          <button
            key={getId()}
            className={`h-10 min-w-[120px] sm:w-auto sm:px-5 sm:!text-left capitalize hover:text-amber-900 focus:outline-none focus:text-amber-900 ${
              i === activeExperience ? 'text-amber-900' : ''
            }`}
        ...
        <div
          style={sliderStyle}
          className={`absolute h-0.5 w-[120px] sm:w-0.5 sm:h-10 rounded-full bg-amber-900 bottom-0 left-0 sm:inset-0 transition-all duration-250 delay-100 in-scroll`}
        ></div>
      </div>
      ...
      <div key={getId()} className="space-y-5 p-1">
        <div className="space-y-1">
          <h3 className="text-lg font-medium text-dark-2 capitalize">
            {role}{' '}
            <Link href={companyUrl} target="_blank" className="text-amber-900">
              @{company}
            </Link>
          </h3>
